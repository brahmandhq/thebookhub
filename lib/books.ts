...
export const getBooksByGenreId = async (genreId: number) => {
  try {
    ...
    throw new Error(
      "An error occurred while fetching books by genre ID. Please try again."
    );
  }
};
...
export const getGenreNameById = async (genreId: number) => {
  try {
    ...
    throw new Error(
      "An error occurred while fetching the genre name by ID. Please try again.",
      error as any
    );
  }
};
...
export const getBooks = async ({
  page,
  limit = 10,
}: { page?: string | number | null; limit?: string | number | null } = {}) => {
  try {
...
export const getBooksByAuthorOrTitle = async (
...
export async function getBooksForClient({
  page,
  limit = 10,
}: { page?: string | number | null; limit?: string | number | null } = {}) {
  try {
    ...
    const response = await fetch(
      `/api/books/${page!.toString()}/${limit!.toString()}`,
      {
        method: "GET",
      }
    );
...
export const getBooksByGenre = cache(
  async (category: string, offset: number = 0, batchSize: number = 25) => {
    try {
...
export const getUniqueGenres = cache(async () => {
  try {
    ...
    throw new Error(
      "An error occurred while fetching unique genres. Please try again."
    );
  }
});
